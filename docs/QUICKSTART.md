...
**Optional Variables** (for remote storage only):
```bash
DO_SPACES_KEY=your_spaces_key  # Optional - for DO Spaces upload
DO_SPACES_SECRET=your_spaces_secret  # Optional - for DO Spaces upload

# Note: No external API keys required for training!
# All logging handled by self-hosted TrainingLogger
```
...
- Review logs in `/datasets/marauder-do-bucket/training/logs/` directory
- Check training metrics in JSON files: `cat logs/<step>/metrics.json | jq .`
