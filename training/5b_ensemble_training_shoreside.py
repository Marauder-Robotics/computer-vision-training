...
class ShoreModelTrainer:
    ...
    def _setup_logging(self):
        """Configure logging."""
        log_dir = Path('logs/shore_training')
    ...
    def _init_wandb(self):
        ...
        wandb.init(
            project=wandb_config.get('project', 'marauder-cv-shore'),
            entity=wandb_config.get('entity'),
            name=f'shore_training_{datetime.now().strftime("%Y%m%d_%H%M%S")}',
            config=self.config,
            tags=['shore', 'yolov11x', 'ensemble']
        )
    ...
    def train_variant(
        ...
        try:
            ...
            if wandb.run is not None:
                wandb.log({f'{variant}_{k}': v for k, v in metrics.items()})
    ...
    def validate_model(
        ...
        try:
            ...
            if wandb.run is not None:
                wandb.log({f'{variant}_val_{k}': v for k, v in val_metrics.items()})
